...
// SERVER 1
$SERVER_NAME = 'Swords 1';
$DB_ADDRESS = 'localhost';
$DB_USERNAME = 'root';
$DB_PASSWORD = '@Sm123#123#';
$DB_ACCOUNT = 'paysys';
...
function register_user($db){
  ...
  $password_sec1 = mysqli_real_escape_string($db, $_POST['password_sec1']);
  $password_sec2 = mysqli_real_escape_string($db, $_POST['password_sec2']);
  ...
  if ($password_sec1 != $password_sec2) {
    array_push($errors, "The two secondary passwords do not match");
  }

  $user_check_query = "SELECT * FROM user_web WHERE username='$username' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  $user = mysqli_fetch_assoc($result);

  $email_check = "SELECT * FROM user_web WHERE email = '$email' LIMIT 1";
  $result2 = mysqli_query($db, $email_check);
  $webemail = mysqli_fetch_assoc($result2);

  $phone_check = "SELECT * FROM user_web WHERE phonenumber = '$phone' LIMIT 1";
  $result1 = mysqli_query($db, $phone_check);
  $webphone = mysqli_fetch_assoc($result1);
  ...
  if ($webemail) { // if user exists
    if ($webemail['email'] === $email) {
      array_push($errors, "Email already exists");
    }
  }

  if ($webphone) { // if user exists
    if ($webphone['phonenumber'] === $phone) {
  ...
  if (count($errors) == 0) {
    ...
    $passwordsec = md5($password_sec1);
	global $coin;
  	$query = "INSERT INTO user_web (username, password, secondary, email, phonenumber) 
  			  VALUES('$username', '$password', '$passwordsec', '$email', '$phone')";
...
function register_acc($db) {

  // receive all input values from the form
  $username = mysqli_real_escape_string($db, $_POST['username_acc']);
  $password_1 = mysqli_real_escape_string($db, $_POST['password_1_acc']);
  $password_2 = mysqli_real_escape_string($db, $_POST['password_2_acc']);
  ...
  // Retrieve the username from 'user_web'
  $user_web_query = "SELECT username FROM user_web WHERE username='$username' LIMIT 1";
  $user_web_result = mysqli_query($db, $user_web_query);
  $user_web_data = mysqli_fetch_assoc($user_web_result);
  ...
  if (count($errors) == 0) {
      ...
      global $coin,$email;
      // Insert into 'account' table using the retrieved username from the session
      $query = "INSERT INTO account (username, secpassword, password, rowpass, coin, email, webuser) 
                VALUES('$username', '$password', '$password', '$password_1', '$coin', '$email', '$user1')";
...
//   $password_1 = mysqli_real_escape_string($db, $_POST['password_1_acc']);
//   $password_2 = mysqli_real_escape_string($db, $_POST['password_2_acc']);
//   $user2 = mysqli_real_escape_string($db, $_POST['user1']);
//   $user3 = "";
//   $user1 = $_SESSION['username'];
